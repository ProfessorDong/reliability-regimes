...
Pilot:   python -m reliability.run_fewshot_v1 --targets drd3 --ks 10 --seeds 3 --budget 250
Full:    python -m reliability.run_fewshot_v1 --targets drd3 scd1 --ks 5 10 20 --seeds 25 --budget 300
"""
...
from reliability.oracle import load_target_data, GPCR_DOCKING  # noqa: E402
from reliability.reward import TargetReward  # noqa: E402
from reliability.actions import FragmentLibrary, ActionSpace, AdmissibilityFilter  # noqa: E402
from reliability.dynamics import LatentEmbedder, LatentSurrogate  # noqa: E402
from reliability.planner import Planner  # noqa: E402
...
def run_cell(target, k, seed, budget, chembl_pool, embedder, modes, hp):
    ...
    cell = {}
    for mode in modes:
        rw = TargetReward(target, anti_targets=cfg['anti'], lambda_unc=hp['lambda_unc'], weights=cfg['weights'])
        wm = LatentSurrogate(embedder, n_ensemble=5, device='cuda', seed=seed) if mode == 'surrogate' else None

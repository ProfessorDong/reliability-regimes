...
class Planner:
    def __init__(self, target, reward: TargetReward, action_space: ActionSpace,
                 surrogate: LatentSurrogate | None, admissibility: AdmissibilityFilter,
    ...
    def _acquire(self, candidates):
        ...
        if not candidates:
            return []
        if self.mode == 'random' or self.surrogate is None or not self.surrogate.fitted:
...
if __name__ == '__main__':
    ...
    def make(mode, seed=0):
        ...
        wm = LatentSurrogate(embedder, n_ensemble=5, device=device, seed=seed) if mode == 'wm' else None
    ...
    p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                     'outputs', 'frozen', f'planner_smoke_{args.target}.json')
    with open(p, 'w') as f:

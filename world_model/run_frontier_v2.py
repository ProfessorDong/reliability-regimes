...
support-novelty. Writes frontier_v2_results.json + frontier_v2_analysis.json.
    ...
    python -m world_model.run_frontier_v2 --seeds 15
"""
...
from world_model.oracle import load_target_data, load_oracle, predict_pic50, featurize  # noqa: E402
...
from world_model.run_applicability_v1 import max_tanimoto_to_set  # noqa: E402
...
def run_one(opt, target, reward, wm, seeds):
    if opt == 'graphga':
        ...
        ga = WMGuidedGA(target, reward, world_model=wm, mode='wm', beta=0.0, seed=0)
    ...
    for target in args.targets:
        ...
        for seed in range(args.seeds):
            ...
            for lam in LAMBDAS:
                for wnov in WNOV:
                    w = dict(cfg['weights']);
                    rw = CWMReward(target, anti_targets=cfg['anti'], lambda_unc=lam,
                    ...
                    for opt in OPTS:
                        ...
                        rwr = CWMReward(target, anti_targets=cfg['anti'], lambda_unc=lam,
                                        weights=w, novelty_anchors=seeds, w_novelty=wnov)

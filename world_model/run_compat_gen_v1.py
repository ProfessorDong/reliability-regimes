...
comparison. Tight budget=150.
    ...
    python -m world_model.run_compat_gen_v1 --targets scd1 fads nk1r drd2 drd3 --seeds 15
"""
...
from world_model.oracle import load_target_data, load_oracle, TARGETS  # noqa: E402
...
def _source_pretrained_wm(embedder, source, seed):
    ...
    rw_s = CWMReward(source, lambda_unc=0.0, weights=WEIGHTS)
...
def _run_T(target, seeds, wm):
    rw = CWMReward(target, lambda_unc=0.1, weights=WEIGHTS)
    ga = WMGuidedGA(target, rw, world_model=wm, mode='wm', seed=hash(tuple(seeds)) % (2**31))

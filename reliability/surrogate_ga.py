...
run(seed_smiles, budget) -> results (same shape as Planner/GraphGA).
    ...
    python -m reliability.surrogate_ga --target drd3 --mode surrogate --budget 300
"""
...
from reliability.graph_ga import crossover, mutate, _csmi  # noqa: E402
from reliability.dynamics import LatentSurrogate  # noqa: E402


class SurrogateTriagedGA:
    def __init__(self, target, reward, surrogate=None, pop_size=60, eval_per_gen=40,
                 pool_mult=5, mut_rate=0.5, beta=1.0, mode='surrogate', seed=0):
        assert mode in ('surrogate', 'randtriage')
        self.target = target
        self.reward = reward
    ...
    def _evaluate(self, smiles):
        ...
        if self.surrogate is not None:
            self.surrogate.update(new, r, epochs=200)
    ...
    def run(self, seed_smiles, budget):
        ...
        while self.reward.n_oracle_calls < budget:
            ...
            if n_take <= 0:
                break
            if self.mode == 'surrogate' and self.surrogate is not None and self.surrogate.fitted:
                mean, std = self.surrogate.predict(pool)
    ...
    def results(self):
        ...
        return dict(target=self.target, method=f'surrogate_ga_{self.mode}',
...
if __name__ == '__main__':
    ...
    from reliability.dynamics import LatentEmbedder
    ap = argparse.ArgumentParser()
    ap.add_argument('--target', default='drd3')
    ap.add_argument('--mode', default='surrogate', choices=['surrogate', 'randtriage'])
    ...
    rw = TargetReward(args.target, lambda_unc=0.05, weights={'potency': 1.0, 'qed': 0.3, 'sa': 0.3})
    wm = LatentSurrogate(LatentEmbedder(device='cuda'), n_ensemble=5, device='cuda', seed=0) \
        if args.mode == 'surrogate' else None
    ga = SurrogateTriagedGA(args.target, rw, surrogate=wm, mode=args.mode, seed=0)
